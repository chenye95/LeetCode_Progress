...
def sliding_puzzle(board: List[List[int]]) -> int:
    ...
    if start_position == target:
        return 0
    stack = [(start_position, zero_position, 0), ]
    visited = {start_position, }
    while stack:
        current_board, zero_position, step_count = stack.pop(0)
        possible_next_steps = []
...
test_cases = [([[1, 2, 3], [4, 0, 5]], 1),
              ...
              ([[4, 1, 2], [5, 0, 3]], 5),
              ([[3, 2, 4], [1, 5, 0]], 14)]
for test_board, expected_count in test_cases:
    assert sliding_puzzle(test_board) == expected_count
