...
def get_max_additional_diners_count_1(N: int, K: int, M: int, S: List[int]) -> int:
    openings = []
    last_seat_right = 1
...
def get_max_additional_diners_count_2(N: int, K: int, M: int, S: List[int]) -> int:
    open_count = 0
    last_seat_right = 1
    ...
    for seat in sorted(S):
        seat_left = seat - K - 1
        if seat_left >= last_seat_right:
            open_count += math.ceil((seat_left - last_seat_right + 1) / (K + 1))
        last_seat_right = seat + K + 1

    if last_seat_right < N:
        open_count += math.ceil((N - last_seat_right + 1) / (K + 1))
...
for get_max_additional_diners_count in [get_max_additional_diners_count_1, get_max_additional_diners_count_2, ]:
    for (test_N, test_K, test_M, test_S), expected_value in test_cases:
        assert get_max_additional_diners_count(test_N, test_K, test_M, test_S) == expected_value, (
            get_max_additional_diners_count.__name__)
